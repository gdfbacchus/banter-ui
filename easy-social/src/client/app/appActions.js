...
export const getRate = () => (dispatch, getState, { steemAPI }) => {
  dispatch({
    ...
    payload: {
      promise: steemAPI
        .sendAsync('condenser_api.get_current_median_history_price', [])
...
export const getRewardFund = () => (dispatch, getSelection, { steemAPI }) =>
  dispatch({
    ...
    payload: { promise: steemAPI.sendAsync('condenser_api.get_reward_fund', ['post']) },
  });
...
export const getTrendingTopics = () => (dispatch, getState, { steemAPI }) => {
  dispatch({
    ...
    payload: {
      promise: steemAPI.sendAsync('condenser_api.get_trending_tags', [undefined, 50]).then(result =>
