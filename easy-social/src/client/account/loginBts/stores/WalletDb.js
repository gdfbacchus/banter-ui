...
import idb_helper from "../utils/idb-helper";
...
class WalletDb extends BaseStore {
    ...
    isLocked() {
      // const res = !(!!aes_private || !!_passwordKey)
      // console.log("!!aes_private : ",!!aes_private );
      // console.log("!!_passwordKey: ",!!_passwordKey);
      // console.log("!!aes_private || !!_passwordKey: ",!!aes_private || !!_passwordKey);
      // console.log("res: ",res);
    ...
    validatePassword(
        ...
        unlock = false,
        account = null,
        roles = ["active", "posting", "owner", "memo"]
    ) {
      return new Promise((resolve, reject)=> {

        // console.log("START validatePassword _passwordKey: ", _passwordKey);
        //debugger;
        if (account) {
          let id = 0;
          ...
          let acc1 = ChainStore.getAccount(account, false);
          ...
          setTimeout(() => {
            ...
            let key;
            if (fromWif) {
              // console.log("pubKey: ", fromWif.toPublicKey().toString());
              key = {
                privKey: fromWif,
            ...
            roles.forEach(role => {
              ...
              if (acc) {
                if (role === "memo") {
                  // console.log("MEMO acc: ",acc.getIn(["options", "memo_key"]));
                  // console.log("MEMO : ",key.pubKey);
                  // console.log("MEMO equals =  : ",acc.getIn(["options", "memo_key"]) === key.pubKey);
                  ...
                  foundRole = true;
                    ...
                    //setKey(role, key.privKey, key.pubKey);
                    foundRole = true;
                  // console.log("POSTING acc: ",acc.getIn(["options", "posting"]));
                  // console.log("POSTING : ", key.pubKey);
                  // console.log("POSTING equals =  : ",acc.getIn(["options", "posting"]) === key.pubKey);
                } else {
                  acc.getIn([role, "key_auths"]).forEach(auth => {
                    // console.log("ROLE["+role+"] acc: ",auth.get(0));
                    // console.log("ROLE["+role+"] : ",key.pubKey);
                    // console.log("ROLE["+role+"] equals =  : ", auth.get(0) === key.pubKey);
            ...
            // console.log("AFTER FOREACH_passwordKey: ", _passwordKey);
            if(_passwordKey) {
              if(memoKeys) {
                setKey(memoKeys.role, memoKeys.privKey, memoKeys.pubKey);
              }
              if(postingKeys) {
                setKey(postingKeys.role, postingKeys.privKey, postingKeys.pubKey);
              }
            }
            ...
            // console.log("IS LOGGED IN: ", !this.isLocked())
            //return {success: !!_passwordKey, cloudMode: true};
            resolve({success: !!_passwordKey, cloudMode: true});
          }, 200)
          ...
          try {
            ...
            let password_pubkey = password_private
              .toPublicKey()
              .toPublicKeyString();
            if (wallet && wallet.password_pubkey && wallet.password_pubkey !== password_pubkey) return false;
            if (unlock) {
              let password_aes = Aes.fromSeed(password);
              let  encryption_plainbuffer = null;
              ...
              aes_private = encryption_plainbuffer ? Aes.fromSeed(encryption_plainbuffer) : null;
            }
            //return {success: true, cloudMode: false};
            resolve({success: true, cloudMode: false});
          } catch (e) {
            console.error(e);
            // return {success: false, cloudMode: false};
            resolve({success: false, cloudMode: false});
          }
        }
      });
    }//(!!aes_private || !!_passwordKey)
      ...
      return new Promise((resolve, reject)=>{
        ...
        if (account && password) {
          ...
          setTimeout(()=>{
            ...
            //console.log("acc: ",acc);console.log("fromWif: ",fromWif);
            ...
            roles.forEach(role => {
              if (!fromWif) {
                // console.log("GENERATE KEYS FOR role: ",role);
                key = this.generateKeyFromPassword(account, role, password);
              }
              ...
              if (acc) {
                if (role === "memo") {
                  ...
                  // console.log('acc.getIn(["options", "memo_key"]: ', acc.getIn(["options", "memo_key"]) );
                  // console.log("key.pubKey: ", key.pubKey);
                  // console.log('acc.getIn(["options", "memo_key"]) === key.pubKey: ', acc.getIn(["options", "memo_key"]) === key.pubKey);
                  // console.log("-----------------------------------------");

                  if (acc.getIn(["options", "memo_key"]) === key.pubKey) {
                    // console.log("role: "+role+" | ", acc.getIn(["options", "memo_key"]));
                    //setKey(role, key.privKey, key.pubKey);
                    foundRole = true;
                  }
                }
                  ...
                  // console.log("role: "+role);
                  acc.getIn([role, "key_auths"]).forEach(auth => {
                    // console.log("auth.get(0): ",auth.get(0));
                    // console.log("key.pubKey: ",key.pubKey);
                    // console.log("(auth.get(0) === key.pubKey): ",(auth.get(0) === key.pubKey));
                    // console.log("-----------------------------------------");

                    if (auth.get(0) === key.pubKey) {
                      passedRroles[role] = true;
                      //setKey(role, key.privKey, key.pubKey);
                      foundRole = true;
                      return false;
                    }
                  ...
                  if (!foundRole) {
                    ...
                    // console.log("role(alsoCheckRole): ",alsoCheckRole);
                    ...
                    acc.getIn([alsoCheckRole, "key_auths"]).forEach(auth => {
                      ...
                      //console.log("role(alsoCheckRole): "+alsoCheckRole+" | ", auth);
            ...
            const isValid = !!(passedRroles["owner"] && passedRroles["owner"] === true && passedRroles["active"] && passedRroles["active"] === true);
            //console.log("passedRroles: ", passedRroles);
