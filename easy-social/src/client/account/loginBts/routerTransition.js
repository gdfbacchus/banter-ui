...
class RouterTransitioner {
    ...
    willTransitionTo(appInit = true, statusCallback = () => {}) {
        //console.log("INIT BTS CONNECTION!");
        // return;
    ...
    _getIndexDBImpl() {
      ...
      if(typeof window != 'undefined' && typeof window.openDatabase !='undefined') {
        return shimIndexedDB || indexedDB
      } else {
        return indexedDB
      }
      ...
      //debugger;
        ...
        try {
            if (chainChanged) {
              //debugger;
                iDB.close();
                const idxDBImpl = this._getIndexDBImpl();
                //console.log("idxDBImpl: ",idxDBImpl)
                dbPromise = iDB.init_instance(idxDBImpl)
                    .init_promise;
            }
        } catch (err) {
