...
export const getUserComments = ({ username, limit = 20 }) => (dispatch, getState, { steemAPI }) =>
  dispatch({
    ...
    payload: steemAPI
      .sendAsync('condenser_api.get_discussions_by_comments', [{ start_author: username, limit }])
...
export const getMoreUserComments = ({ username, limit = 20 }) => (
  ...
  return dispatch({
    ...
    payload: steemAPI
      .sendAsync('condenser_api.get_discussions_by_comments', [
        {
          start_author: startAuthor,
          start_permlink: startPermlink,
          limit: limit + 1,
...
export const getReplies = () => (dispatch, getState, { steemAPI }) => {
  ...
  dispatch({
    ...
    payload: steemAPI
      .sendAsync('condenser_api.get_state', [`/@${category}/recent-replies`])
...
async function getBookmarksData(bookmarks, steemAPI) {
  ...
  for (let idx = 0; idx < Object.keys(bookmarks).length; idx += 1) {
    ...
    const postData = steemAPI.sendAsync('condenser_api.get_content', [
