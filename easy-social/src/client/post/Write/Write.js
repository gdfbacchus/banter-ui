...
import {
  ...
  getUpvoteSetting,
  getRewardSetting,
  getStausSendingPostOrComment,
  getPosts
} from '../../reducers';

import { createPost, saveDraft, newPost } from './editorActions';
import { postCommentResetStatus } from '../../post/postActions.js';
...
@connect(
  (state, props) => ({
    ...
    getStausSendingPostOrComment: getStausSendingPostOrComment(state),
  }),
  {
    createPost,
    saveDraft,
...
class Write extends React.Component {
  ...
  componentDidMount() {
    //console.log("WRITE props: ",this.props)
    this.props.newPost();
    const { draftPosts, draftId } = this.props;
    // const draftPost = draftPosts[draftId];
    const draftPost = this.props.posts[draftId];

    if (draftPost) {
      console.log("EDIT POST props before update: ",draftPost);
      let metadata = JSON.parse(draftPost.json_metadata);
      // console.log("POST EDIT metadata: ",metadata);
      let tags = [];
      if (metadata && _.isArray(metadata.tags)) {
        tags = metadata.tags;
      }
      ...
      if (draftPost.body) {
        this.originalBody = draftPost.body;
      }
  ...
  onSubmit = (form, wif, isUpdating)=> {
    ...
    this.props.createPost(data,wif);
  };
