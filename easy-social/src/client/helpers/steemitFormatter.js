  ...
  function estimateAccountValue(
    ...
    if (!vesting_steem || !feed_price) {
      if (!gprops || !feed_price) {
        promises.push(
          api.sendAsync('condenser_api.get_state', [`/@${username}`])
    ...
    if (!open_orders) {
      promises.push(
        api.sendAsync('condenser_api.get_open_orders', [username]).then(open_orders => {
    ...
    if (!savings_withdraws) {
      promises.push(
        api.sendAsync('condenser_api.get_savings_withdraw_from', [username]).then(savings_withdraws => {
