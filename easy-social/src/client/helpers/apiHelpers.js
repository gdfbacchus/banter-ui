...
export function getDiscussionsFromAPI(sortBy, query, steemAPI) {
  switch (sortBy) {
    ...
    case 'comments':
    case 'promoted':
      // console.log("getDiscussionsFromAPI sortBy: ",sortBy);
      // console.log("getDiscussionsFromAPI query: ",query);
      // console.log("-------------------------------------");

      return steemAPI.sendAsync(`condenser_api.get_discussions_by_${sortBy}`, [query]);
    default:
...
export const getAccount = username =>
  SteemAPI.sendAsync('condenser_api.get_accounts', [[username]]).then(result => {
    ...
    if (result.length) {
      ...
      // debugger;
      console.log("Username["+username+"] not found: ",result);
...
export const getFollowingCount = username =>
  ...
  SteemAPI.sendAsync('condenser_api.get_follow_count', [username]);
...
export const getFollowing = (username, startForm = '', type = 'blog', limit = 100) =>
  ...
  // ])
SteemAPI.sendAsync('condenser_api.get_following', [username, startForm, type, limit])
    .then(result => {
      // console.log("["+username+"]get following: ",result);
...
export const getFollowers = (username, startForm = '', type = 'blog', limit = 100) =>
  ...
  //   [username, startForm, type, limit],
  // ])
  SteemAPI.sendAsync('condenser_api.get_followers', [username, startForm, type, limit])
    .then(result => {
      // console.log("["+username+"]followers: ",result);
      return result.map(user => user.follower)
    });

export const getAllFollowing = username => {
  // console.log("getAllFollowing for ["+username+"]");
  return new Promise(async resolve => {
    ...
    // console.log("HELPER following: ",following);
    // console.log("HELPER limitArray: ",limitArray);
    ...
    // console.log("HELPER final list: ",list.then((res)=>{console.log("RES: ",res)}));
    resolve(list);
  });
}
...
export const getAccountHistory = (account, from = -1, limit = defaultAccountLimit) =>
  SteemAPI.sendAsync('condenser_api.get_account_history', [account, from, limit]);

export const getDynamicGlobalProperties = () =>
  SteemAPI.sendAsync('condenser_api.get_dynamic_global_properties', []);
...
export const getAccountReputation = (name, limit = 20) =>
  ...
  SteemAPI.sendAsync('condenser_api.get_account_reputations', [[name, limit]]);
...
export const currentUserFollowersUser = (currentUsername, username) =>
  ...
  // ]);
SteemAPI.sendAsync('condenser_api.get_following', [username, currentUsername, 'blog', 1]);
