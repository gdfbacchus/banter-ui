...
class StoryFull extends React.Component {
  ...
  componentDidMount(){
    ...
    // console.log("Post P author: ", author);
    getAccount(author).then((resp)=>{
      ...
      let profileSignature =
        user.json_metadata
        && user.json_metadata.profile
        && user.json_metadata.profile.signature ? user.json_metadata.profile.signature : '';

      //console.log("profileImage: ",profileImage)
      this.setState({profileImage, profileSignature})
    });
  }
  ...
  render() {
    ...
    } else if(this.state.profileSignature) {
      signedBody = `${post.body}<hr>${this.state.profileSignature}`;
    }
    ...
    return (
      <div className="StoryFull">
        ...
        <div className="StoryFull__header">
          <Link to={`/@${post.author}`}>
            <Avatar username={post.author} size={60} profileImage={this.state.profileImage} />
          </Link>
